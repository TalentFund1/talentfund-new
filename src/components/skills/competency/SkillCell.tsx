...
export const SkillCell = ({ details, isLastColumn }: SkillCellProps) => {
  ...
  const getLevelStyles = (level: string) => {
    const baseStyles = 'rounded-t-md px-1 py-1.5 text-sm font-medium w-full capitalize flex items-center justify-center min-h-[26px] text-[#1f2144]';
  ...
  const getRequirementStyles = (requirement: string, level: string) => {
    ...
    const baseStyles = 'text-xs px-1 py-1.5 font-medium text-[#1f2144] w-full flex items-center justify-center gap-1.5';
  ...
  return (
    <TableCell 
      className={`text-center p-0.5 align-middle ${!isLastColumn ? 'border-r' : ''} border-border`}
    >
