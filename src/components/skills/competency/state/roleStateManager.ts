...
export const getRoleStateKey = (roleId: string) => `competency-states-${roleId}`;

export const loadRoleState = (roleId: string): Record<string, Record<string, SkillState>> => {
  try {
    const key = getRoleStateKey(roleId);
...
export const saveRoleState = (
  roleId: string, 
  state: Record<string, Record<string, SkillState>>
) => {
  try {
    const key = getRoleStateKey(roleId);
