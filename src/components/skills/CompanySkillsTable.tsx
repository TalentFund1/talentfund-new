...
export const CompanySkillsTable = () => {
  ...
  // Helper function to get the harmonized category
  const getHarmonizedCategory = (skillTitle: string): string => {
    // Check in roleSkills to determine the category
  ...
  // Helper function to determine weight based on growth and category
  const getSkillWeight = (skill: any): string => {
    const growthValue = parseFloat(skill.growth);
    if (growthValue >= 25) return 'Critical';
    if (skill.type === 'technical') return 'Technical';
    return 'Necessary';
  };
            ...
            <TableBody>
              {skills.map((skill, index) => (
                <TableRow key={`${skill.title}-${index}`} className={index % 2 === 0 ? "bg-muted/5" : ""}>
                  ...
                  <TableCell>{getSkillWeight(skill)}</TableCell>
