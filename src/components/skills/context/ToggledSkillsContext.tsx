...
const getStorageKey = (roleId: string) => `roleToggledSkills-${roleId}-v3`;
...
export const ToggledSkillsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [toggledSkills, setToggledSkills] = useState<Set<string>>(() => {
    try {
      ...
      console.error('Error initializing toggled skills:', error);
      return new Set();
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedSkills.length === 0) {
        ...
        if (initialSkills.length > 0) {
          ...
          toast({
            ...
            description: `${initialSkills.length} default skills loaded for this role.`,
          });
        }
      } else {
        ...
        toast({
          title: "Skills Loaded",
          description: `${savedSkills.length} saved skills restored.`,
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "There was an error loading your saved skills.",
        variant: "destructive",
      });
    }
  }, [selectedRole, id, initialized, toast]);
  ...
  const handleSetToggledSkills = (newSkills: Set<string>) => {
    try {
      ...
      console.error('Error setting toggled skills:', error);
      toast({
